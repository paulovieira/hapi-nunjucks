...
exports.register = function (server, pluginOptions, next) {
    ...
    server.decorate('server', 'nunjucksViews', function (options) {

        Hoek.assert(options, 'Missing views options');
        this.realm.plugins['hapi-nunjucks'] = this.realm.plugins['hapi-nunjucks'] || {};
        Hoek.assert(!this.realm.plugins['hapi-nunjucks'].manager, 'Cannot set views manager more than once');
        ...
        this.realm.plugins['hapi-nunjucks'].manager = manager;
        return manager;
    });
    ...
    server.decorate('reply', 'view', function (template, context, options) {
        ...
        const realm = (this.realm.plugins['hapi-nunjucks'] || this.request.server.realm.plugins['hapi-nunjucks'] || {});
